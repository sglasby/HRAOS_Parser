...
namespace HRAOS_take2
{
    partial class Script_Parser
    {
        ...
        public int parse_OBJ_parent_archetype_field()
        {
            ...
            int ID = Object_Registrar.All.ID_for_tag(tag_name);
            if (ID == 0) { error("Got invalid ARCHETYPE tag '{0}' for OBJ", tag_name); }
            tmp.parent_archetype = Object_Registrar.All.Archetype_for_ID(ID);
        ...
        public int parse_OBJ_tag_field()
        {
            ...
            if (Object_Registrar.All.ID_for_tag(tag_name) != 0) { error("Got already-used TAG '{0}' in OBJ preamble", tag_name); }
        ...
        public int parse_OBJ_ID_field()
        {
            ...
            if (Object_Registrar.All.is_valid_ID(ID)) { error("Got already in-use ID '{0}' in OBJ preamble", ID); }
            tmp.ID = ID;
